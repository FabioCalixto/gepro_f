...
class AdmsValNip {
    ...
    public function valNipInfractor($Infractor, $EditarUnico = null, $DadoId = null)
    {
        ...
        if(!empty($this->EditarUnico) AND ($this->EditarUnico == true)){
            $valNipInfractor->fullRead("SELECT id FROM tab_infratores WHERE nip =:nip AND id <>:id LIMIT :limit", "nip={$this->Infractor}&limit=1&id={$this->DadoId}");            
        }else{
            $valNipInfractor->fullRead("SELECT id FROM tab_infratores WHERE nip =:nip LIMIT :limit", "nip={$this->Infractor}&limit=1");
        }        
