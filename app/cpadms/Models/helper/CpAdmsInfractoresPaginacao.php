 ...
 */
class CpAdmsInfractoresPaginacao
{
    ...
    private function layoutPaginacao()
    {
        ...
        $this->Resultado .= '<a class="page-link" href="#" tabindex="-1" onclick="listar_infractor(1, '.$this->Var.')">Primeira</a>';
        $this->Resultado .= "</li>";
        for ($iPag = $this->Pagina - $this->MaxLinks; $iPag <= $this->Pagina - 1; $iPag++) {
            if ($iPag >= 1) {
                $this->Resultado .= '<li class="page-item"><a class="page-link" href="#" onclick="listar_infractor('.$iPag.', '.$this->Var.')">'.$iPag.'</a></li>';
            }
        }
        ...
        for ($dPag = $this->Pagina + 1; $dPag <= $this->Pagina + $this->MaxLinks; $dPag++) {
            if ($dPag <= $this->TotalPaginas) {
                $this->Resultado .= '<li class="page-item"><a class="page-link" href="#" onclick="listar_infractor('.$dPag.', '.$this->Var.')">'.$dPag.'</a></li>';
            }
        }
        $this->Resultado .= "<li class='page-item'>";
        $this->Resultado .= '<a class="page-link" href="#" onclick="listar_infractor('.$this->TotalPaginas.', '.$this->Var.')">Última</a>';
        $this->Resultado .= "</li>";
        $this->Resultado .= "</ul>";
        $this->Resultado .= "</nav>";
    }
